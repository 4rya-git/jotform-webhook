...
app.post('/webhook', upload.none(), async (req, res) => {
    try {
        ...
        for (const orderLine of mappedResult) {
            const formattedProductName = `${orderLine.product_name} (${orderLine.product_options.slice(1).join(', ')})`;
            const productId = await findOrCreateProduct(formattedProductName, orderLine.unit_price);
            odooOrderLines.push([
                0, 0, {
                    product_id: productId || orderLine.product_id,
                    name: orderLine.formattedProductName,
                    product_uom_qty: orderLine.quantity,
                    price_unit: orderLine.unit_price
                }
            ]);
        }
